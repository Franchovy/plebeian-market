...
export function InfiniteProductList({
	...
	if (isLoading && products.length === 0) {
		return (
			<div className={cn('w-full max-w-full overflow-hidden', className)} ref={containerRef}>
				{title && (
					<div className="mb-4">
						{typeof title === 'string' ? <h1 className="font-heading text-xl sm:text-2xl sm:text-left text-center">{title}</h1> : title}
					</div>
				)}

				<div className="flex flex-col justify-center items-center py-12 min-h-[60vh]">
	...
	if (products.length === 0) {
		return (
			<div className="flex flex-col justify-center items-center py-12 text-center">
				<h3 className="mb-2 font-semibold text-gray-900 text-lg">No products found</h3>
	...
	return (
		<div className={cn('w-full max-w-full overflow-hidden', className)} ref={containerRef}>
			...
			{/* Load more button */}
			{hasMore && (
				<div className="flex justify-center items-center py-8 min-h-[80px]">
