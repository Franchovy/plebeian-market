...
import { WotBadge } from '@/components/features/header/WotScore'
	...
	return (
		<div className="relative w-full">
			...
			{showResults && (
				<div
					ref={resultsRef}
					className="top-full z-50 absolute bg-white shadow-lg mt-1 border border-gray-200 rounded-md w-full overflow-hidden"
				>
					<div className="p-1 max-h-[300px] overflow-y-auto">
						{eventList.length === 0 ? (
							<div className="p-2 text-gray-500 text-sm text-center">{isLoading ? 'Searching...' : 'No profiles found.'}</div>
						) : (
							<div>
								{eventList.map((event) => {
									let profile = null
									...
									return (
										<div
											key={event.id}
											className="group flex items-center gap-2 hover:bg-gray-100 p-2 rounded cursor-pointer"
										...
										>
											...
											>
												<Button variant="ghost" size="icon" className="rounded-full w-8 h-8" title="View profile">
