...
export function Nip60Wallet() {
	...
	const classNameButtonGhost = 'bg-transparent hover:bg-white/10 text-gray-400 hover:text-white border-0'
	const classNameButtonSuccess = 'bg-green-600 hover:bg-green-700 text-white border-0'
	const classNameButtonWarning = 'bg-orange-600 hover:bg-orange-700 text-white border-0'
	const classNameButtonMuted = 'bg-transparent hover:bg-white/10 text-gray-400 hover:text-white border-0'
	const classNameButtonActive = 'bg-white/20 text-white border-0'
	const classNameButtonSubtle = 'bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white border-0'
	const classNameButtonDestructive = 'bg-transparent hover:bg-red-500/20 text-red-400 hover:text-red-300 border-0'
	const classNameButtonSectionOpen = classNameButtonActive + ' flex-1 gap-1.5 px-2'
	const classNameButtonSectionInactive = classNameButtonSubtle + ' flex-1 gap-1.5 px-2'
		...
		<div className="bg-primary p-4 rounded-lg max-w-full overflow-hidden text-white">
			<div className="relative mb-4 text-center">
				<div className="top-0 right-0 absolute flex gap-1">
					<Button
						className={classNameButtonGhost}
						size="icon"
			...
			<div className="gap-2 grid grid-cols-2 mb-4">
				<Button className={classNameButtonSuccess} size="sm" onClick={() => setOpenModal('deposit')} icon={<Zap className="w-4 h-4" />}>
					Deposit
				</Button>
				<Button className={classNameButtonWarning} size="sm" onClick={() => setOpenModal('withdraw')} disabled={balance === 0}>
					<Zap className="w-4 h-4" />
					Withdraw
				</Button>
				<Button className={classNameButtonMuted} size="sm" onClick={() => setOpenModal('receive')}>
					<QrCode className="w-4 h-4" />
					Receive eCash
				</Button>
				<Button className={classNameButtonMuted} size="sm" onClick={() => setOpenModal('send')} disabled={balance === 0}>
			...
			<div className="pt-2 overflow-hidden">
				...
				<div className="flex gap-1 mb-2">
					<Button
						className={openSection === 'mints' ? classNameButtonSectionOpen : classNameButtonSectionInactive}
						size="sm"
					...
					<Button
						className={openSection === 'transactions' ? classNameButtonSectionOpen : classNameButtonSectionInactive}
						size="sm"
					...
					<Button
						className={openSection === 'proofs' ? classNameButtonSectionOpen : classNameButtonSectionInactive}
						size="sm"
					...
					{activePendingTokens.length > 0 && (
						<Button
							className={openSection === 'pending' ? classNameButtonSectionOpen : classNameButtonSectionInactive}
							size="sm"
				...
				{openSection === 'mints' && (
					<div className="space-y-2 pt-2 border-white/10 border-t overflow-hidden">
						{mints.map((mint) => (
							<div key={mint} className="flex justify-between items-center gap-2 text-sm">
								...
								<div className="flex items-center gap-1 shrink-0">
									...
									<Button
										className={classNameButtonGhost + ' w-6 h-6'}
										size="icon"
						...
						<div className="flex gap-2">
							...
							/>
							<Button
								className={classNameButtonMuted + ' px-2 h-8 shrink-0'}
								size="sm"
						...
						<Button className={classNameButtonActive + ' w-full'} size="sm" onClick={handleSaveWallet} disabled={isSaving}>
								...
								<Collapsible key={mint} open={expandedMints.has(mint)} onOpenChange={() => toggleMintExpanded(mint)}>
									<div className="bg-white/5 p-2 rounded-md overflow-hidden">
										<CollapsibleTrigger asChild>
											<Button className={classNameButtonGhost + ' justify-start gap-2 px-1 py-1 w-full h-auto overflow-hidden'} size="sm">
						...
						{activePendingTokens.map((token) => (
							<div key={token.id} className="flex justify-between items-center gap-2 bg-white/5 p-2 rounded-lg">
								...
								<div className="flex gap-0.5 shrink-0">
									<Button
										className={classNameButtonGhost + ' w-7 h-7'}
										size="icon"
									...
									<Button
										className={classNameButtonDestructive + ' w-7 h-7'}
										size="icon"
