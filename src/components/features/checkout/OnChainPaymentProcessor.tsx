		...
		return (
			<Card className="opacity-50">
				<CardContent className="p-4">
					<div className="text-gray-500 text-center">On-chain payment processor disabled</div>
				</CardContent>
			</Card>
		)
	}
	...
	return (
		<Card>
			<CardHeader className="pb-3">
				<CardTitle className="flex justify-between items-center">
					<div className="flex items-center gap-2">
						<div className="bg-orange-500 rounded-full w-5 h-5" />
					...
					<div className="text-right">
						...
						<div className="text-gray-500 text-sm">{amountBtc.toFixed(8)} BTC</div>
					</div>
				</CardTitle>
				{checkAttempts > 0 && <div className="text-gray-500 text-sm">Check attempts: {checkAttempts}</div>}
			</CardHeader>
			...
			<CardContent className="space-y-4">
				...
				{paymentStatus !== 'completed' && (
					<div className="space-y-3">
						...
						<div className="space-y-2">
							<div className="font-medium text-sm">Bitcoin Address:</div>
							<div className="bg-gray-50 p-3 rounded">
								<div className="flex items-center gap-2">
									<code className="flex-1 font-mono text-xs break-all">{address}</code>
						...
						<div className="bg-blue-50 p-3 rounded">
							<div className="text-center">
								...
								<div className="text-gray-600 text-sm">{amountBtc.toFixed(8)} BTC</div>
								{label && <div className="mt-1 text-gray-600 text-sm">{label}</div>}
							</div>
						</div>
					</div>
				)}
				...
				{/* Error display */}
				{error && (
					<div className="flex items-center gap-2 bg-red-50 p-3 rounded text-red-600">
				...
				{paymentStatus === 'completed' && lastTransaction && (
					<div className="space-y-3">
						<div className="flex items-center gap-2 bg-green-50 p-3 rounded text-green-600">
						...
						<div className="bg-gray-50 p-3 rounded">
							...
							{lastTransaction.status.confirmed && <div className="mt-1 text-green-600 text-xs">✓ Confirmed</div>}
						</div>
					</div>
				)}
					...
					{paymentStatus !== 'completed' && paymentStatus !== 'expired' && (
						<>
							...
							<Button onClick={handleManualCheck} className="w-full" disabled={isManualChecking}>
								{isManualChecking ? <RefreshCw className="mr-2 w-4 h-4 animate-spin" /> : <Check className="mr-2 w-4 h-4" />}
