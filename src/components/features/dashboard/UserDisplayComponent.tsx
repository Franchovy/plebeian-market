			...
			<div className="flex justify-between items-center gap-4">
				...
				{customActions ? (
					...
					<div className="flex flex-col gap-1">
						{onMoveUp && (
							<Button variant="outline" size="sm" onClick={onMoveUp} disabled={!canMoveUp || isReordering} className="p-0 w-8 h-8">
								<ChevronUp className="w-4 h-4" />
							</Button>
						)}
						{onMoveDown && (
							<Button variant="outline" size="sm" onClick={onMoveDown} disabled={!canMoveDown || isReordering} className="p-0 w-8 h-8">
								<ChevronDown className="w-4 h-4" />
							</Button>
						)}
						{onRemove && (
							<Button variant="destructive" size="sm" onClick={onRemove} disabled={isRemoving} className="p-0 w-8 h-8">
